export const drawSkeleton = (ctx, pose, videoWidth, videoHeight, selectedExercise, angle) => {
  ...
  if (angle !== null) {
      const angleType = selectedExercise === 'Squats' ? 'Knee' : (selectedExercise === 'Push-ups' ? 'Elbow' : 'Elbow');
      ctx.fillStyle = 'white';
      ctx.font = '20px Arial';
...
export const analyzePose = (pose, selectedExercise, setAngle, exerciseStateRef, setExerciseState, setCount, setFeedback) => {
  ...
  if (selectedExercise === 'Squats') {
      ...
      newAngle = analyzeBicepCurls(pose, exerciseStateRef, setExerciseState, setCount, setFeedback);
  }
  setAngle(newAngle);
};
...
const analyzeBicepCurls = (pose, exerciseStateRef, setExerciseState, setCount, setFeedback) => {
  const leftShoulder = pose.keypoints.find(kp => kp.name === 'left_shoulder');
  const leftElbow = pose.keypoints.find(kp => kp.name === 'left_elbow');
  const leftWrist = pose.keypoints.find(kp => kp.name === 'left_wrist');
  ...
  if (leftShoulder && leftElbow && leftWrist) {
      ...
      if (angle < 45 && exerciseStateRef.current === 'down') {
          setExerciseState('up');
          exerciseStateRef.current = 'up';
          setCount(prevCount => prevCount + 1);
      } else if (angle > 160 && exerciseStateRef.current === 'up') {
          setExerciseState('down');
          exerciseStateRef.current = 'down';
      }
